...
function emptyForm() {
  return { batch_no: "", particle_size: "Fine", acm_rpm: "", classifier_rpm: "", blower_rpm: "", remarks: "" };
}
...
function ProductionBatchSetupPage() {
  ...
  const onSubmit = async (event) => {
    ...
    setSaving(true);
    try {
      const payload = buildSectionPatchPayload(record, "pulveriserRpm", form.classifier_rpm, {
      ...
      <section className="order-card">
        ...
        <form onSubmit={onSubmit} className="order-form-grid">
          <div>
            ...
            <input className="input" value={form.batch_no} onChange={(e) => setField("batch_no", e.target.value)} required disabled={!canManageProduction} />
          </div>
          <div>
            <label>Particle Size</label>
            <input className="input" value={form.particle_size} onChange={(e) => setField("particle_size", e.target.value)} disabled={!canManageProduction} />
          </div>
          <div>
            <label>ACM-RPM</label>
            <input className="input" type="number" min="1" value={form.acm_rpm} onChange={(e) => setField("acm_rpm", e.target.value)} disabled={!canManageProduction} />
          </div>
          <div>
            <label>Pulveriser-RPM</label>
            <input className="input" placeholder="e.g. Full / 1500" value={form.classifier_rpm} onChange={(e) => setField("classifier_rpm", e.target.value)} disabled={!canManageProduction} />
          </div>
          <div>
            <label>Blower-RPM</label>
            <input className="input" type="number" min="1" value={form.blower_rpm} onChange={(e) => setField("blower_rpm", e.target.value)} disabled={!canManageProduction} />
          </div>
