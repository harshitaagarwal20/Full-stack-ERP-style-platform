-- ============================================================
-- Nimbasia ERP — ADDITIVE schema update for a LIVE database.
--
-- Purpose: bring an existing database (with real data) up to the
-- current schema by ADDING only what is missing. It is fully
...
-- row, so no existing data can be lost. Safe to re-run.
--
-- Generated from prisma/hostinger-init.sql. Uses PREPARE/EXECUTE
-- guards (no DELIMITER) so it imports cleanly via phpMyAdmin too.
--
