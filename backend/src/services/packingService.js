...
export async function createPackingRecord(payload, actorUser) {
  ...
  const availableFinishedGoods = await getAvailableInventoryQty(order.product);
  ...
  const availableMaterialQty = await getAvailableInventoryQty(packingMaterialItemId);
  ...
  const record = await prisma.$transaction(async (tx) => {
    ...
    await recordAuditEvent({
      ...
      user: actorUser,
      newValue: created,
      note: `Packed ${packedQuantity} of order #${payload.order_id} using ${packingMaterialQty} of "${packingMaterialItemId}"`
    });
